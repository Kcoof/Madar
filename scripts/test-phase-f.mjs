...
async function main() {
  ...
  // --- 2c. subscription activation notification
  ...
  await call(student, "/api/student/subscriptions/request", {
    method: "POST",
  ...
  const req = (pendingList.body?.pending ?? []).find(
    (p) => p.student.email === "student@school-a.local" && p.subject?.name === "العلوم"
  );
  const beforeAct = await call(student, "/api/notifications");
  ...
  const afterAct = await call(student, "/api/notifications");
  ...
  const marked = await call(student, `/api/notifications/${unread.id}/read`, { method: "PATCH" });
